...
export const QuickRejectModal: React.FC<QuickRejectModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md font-sans">
      <div className="w-full max-w-md bg-[#0a0f28] border border-red-500/30 rounded-3xl p-6 shadow-2xl relative space-y-5 animate-scale-up">
        <button
          onClick={onClose}
          className="absolute top-4 right-4 text-white/50 hover:text-white p-1 rounded-full bg-white/5 hover:bg-white/10"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex items-center gap-3 pt-2">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 h-[42px] rounded-full border border-white/15 text-white/70 hover:text-white font-space text-xs font-bold hover:bg-white/5 cursor-pointer"
            >
              Cancel
            </button>
            <button
              type="submit"
              className="flex-1 h-[42px] rounded-full bg-red-600 hover:bg-red-500 text-white font-space text-xs font-bold flex items-center justify-center gap-2 shadow-lg shadow-red-600/30 cursor-pointer"
            >
